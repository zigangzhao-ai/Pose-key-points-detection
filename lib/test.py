...
def test_net(tester, dets, det_range, gpu_id):
    ...
    pbar.set_description("GPU %s" % str(gpu_id)) ##pbar.set_description("gpu %s" % str(gup_id))
...
def test(test_model):
    ...
    # human bbox load
    if cfg.useGTbbox and cfg.testset in ['train', 'test']:
        if cfg.testset == 'trian':
    ...
    #d.evaluation(annot, cfg.result_dir, cfg.testset)
